...
// ==========================================
// SECTION 1: AUTHENTICATION & CORE LOGIC
// ==========================================
...
// ==========================================
// SECTION 2: GRID & MATCHMAKING LOGIC
// ==========================================
...
// ==========================================
// SECTION 3: REQUEST HANDLING HELPERS
// ==========================================
    ...
    if (sscanf(packet->payload, "%31s %63s", username, password) == 2) {
        if (authenticate_user(username, password, current_user)) {
            if (current_user->is_banned) {
                packet->type = MSG_ERROR; strcpy(packet->payload, "Account banned.");
                ...
                if (user_sockets[current_user->user_id] != 0) {
                    ...
                    packet->type = MSG_ERROR; strcpy(packet->payload, "Already logged in elsewhere.");
                    send(client_sock, packet, sizeof(MessagePacket), 0);
                } else {
                    *authenticated = 1;
            ...
            packet->type = MSG_ERROR; strcpy(packet->payload, "Invalid credentials.");
            send(client_sock, packet, sizeof(MessagePacket), 0);
        }
    }
}
...
void handle_ride_request(int client_sock, MessagePacket* packet, UserRecord* current_user) {
    ...
    if (sscanf(packet->payload, "%d %d %d %d", &sx, &sy, &dx, &dy) == 4) {
        int exclude_list[MAX_DRIVERS]; int exclude_count = 0; int trip_started = 0;
        while (!trip_started) {
            int driver_id = request_ride(current_user->user_id, sx, sy, exclude_list, exclude_count);
            if (driver_id == -1) {
                packet->type = MSG_ERROR; strcpy(packet->payload, "No drivers available.");
                send(client_sock, packet, sizeof(MessagePacket), 0); break;
            }
            pthread_mutex_lock(&session_mutex);
            ...
            packet->type = MSG_RIDE_OFFER; sprintf(packet->payload, "%d %d %d %d", sx, sy, dx, dy);
            send(driver_sock, packet, sizeof(MessagePacket), 0);
            
            int wait_time = 0; int local_resp = 0;
            while(wait_time < 10) {
                pthread_mutex_lock(&session_mutex); local_resp = user_responses[driver_id]; pthread_mutex_unlock(&session_mutex);
            ...
            if (local_resp == 1) {
                trip_started = 1; packet->type = MSG_RIDE_MATCHED; sprintf(packet->payload, "%d", driver_id);
                send(client_sock, packet, sizeof(MessagePacket), 0);
                sleep(5); // Simulate trip
                ...
                if (s_fd != -1) {
                    ...
                    if (s_shm != MAP_FAILED) { surge = s_shm->multiplier; munmap(s_shm, sizeof(SurgeState)); }
                    close(s_fd);
                }
...
void handle_admin_action(int client_sock, MessagePacket* packet) {
    char target[32]; int status;
    if (sscanf(packet->payload, "%31s %d", target, &status) == 2) {
        int fd = open("data/users.dat", O_RDWR);
        if (fd == -1) return;
        struct flock lock; memset(&lock, 0, sizeof(lock));
        lock.l_type = F_WRLCK; fcntl(fd, F_SETLKW, &lock);
        FILE *fp = fdopen(fd, "r"); FILE *ftemp = fopen("data/users_temp.dat", "w");
        if (fp && ftemp) {
            char line[256]; int found = 0;
            while (fgets(line, sizeof(line), fp)) {
                int id, r, b; char u[32], p[64];
                if (sscanf(line, "%d %31s %63s %d %d", &id, u, p, &r, &b) == 5 && strcmp(u, target) == 0) {
                    fprintf(ftemp, "%d %s %s %d %d\n", id, u, p, r, status); found = 1;
                } else fputs(line, ftemp);
            }
            fclose(ftemp);
            if (found) {
                rename("data/users_temp.dat", "data/users.dat");
                packet->type = MSG_ADMIN_ACTION; sprintf(packet->payload, "User %s updated.", target);
            } else { remove("data/users_temp.dat"); packet->type = MSG_ERROR; sprintf(packet->payload, "User not found."); }
        }
        lock.l_type = F_UNLCK; fcntl(fd, F_SETLK, &lock); fclose(fp);
        send(client_sock, packet, sizeof(MessagePacket), 0);
    }
}

// ==========================================
// SECTION 4: SYSTEM & NETWORK HANDLING
// ==========================================

void cleanup_and_exit(int sig) {
    (void)sig; printf("Shutting down. Cleaning up IPC resources...\n");
    if (grid_shm) munmap(grid_shm, SHM_SIZE);
    shm_unlink(SHM_NAME); sem_unlink(SEM_POOL_NAME); exit(0);
}
...
void setup_ipc() {
    ...
    for(int i = 0; i < MAX_DRIVERS; i++) { grid_shm->grid[i].status = STATUS_OFFLINE; grid_shm->grid[i].driver_id = 0; }
...
void* handle_client(void* arg) {
    ...
    MessagePacket packet; int authenticated = 0; UserRecord current_user;
    printf("Client connected on Socket %d.\n", client_sock);

    while (1) {
        if (recv(client_sock, &packet, sizeof(MessagePacket), 0) <= 0) {
            if (authenticated && current_user.role == ROLE_DRIVER) update_driver_status(current_user.user_id, STATUS_OFFLINE, 0, 0);
            break;
        }
        ...
        if (packet.type == MSG_AUTH_REQ) {
            ...
            packet.type = MSG_ERROR; strcpy(packet.payload, "Authenticate first.");
            send(client_sock, &packet, sizeof(MessagePacket), 0);
        } else {
            switch(packet.type) {
                case MSG_DISCONNECT:
                    pthread_mutex_lock(&session_mutex); user_sockets[current_user.user_id] = 0; pthread_mutex_unlock(&session_mutex);
                    if (current_user.role == ROLE_DRIVER) update_driver_status(current_user.user_id, STATUS_OFFLINE, 0, 0);
                    goto cleanup;
                case MSG_LOC_UPDATE:
                    if (current_user.role == ROLE_DRIVER) {
                        int s, x, y; if (sscanf(packet.payload, "%d %d %d", &s, &x, &y) == 3) update_driver_status(current_user.user_id, (DriverStatus)s, x, y);
                    }
                    break;
                case MSG_RIDE_ACCEPT:
                    if (current_user.role == ROLE_DRIVER) { pthread_mutex_lock(&session_mutex); user_responses[current_user.user_id] = 1; pthread_mutex_unlock(&session_mutex); }
                    break;
                case MSG_RIDE_REJECT:
                    if (current_user.role == ROLE_DRIVER) { pthread_mutex_lock(&session_mutex); user_responses[current_user.user_id] = 2; pthread_mutex_unlock(&session_mutex); }
                    break;
                case MSG_RIDE_REQ:
...
int main() {
    signal(SIGINT, cleanup_and_exit); setup_ipc();
    ...
    while (1) {
        ...
        *csock = accept(server_sock, NULL, NULL);
        pthread_t tid; pthread_create(&tid, NULL, handle_client, csock); pthread_detach(tid);
    }
    return 0;
}
